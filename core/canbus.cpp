...
Canbus::Canbus(CarStatus *m_carStatus) {
  ...
  if (!device) {
    ...
    // filter.frameId = 0x4ECU;
    // filter.frameIdMask = 0b11111111111U;
  ...
  connect(device, SIGNAL(framesReceived()), this, SLOT(parseSerial()));
...
void Canbus::askStatus() {
  ...
  int ecuControl = carStatus->getEcuControl();
  askErrors[2] = ecuControl;
...
void Canbus::toggleTelemetry(int whatState) {
  QByteArray msg;
  msg.resize(8);
...
void Canbus::parseCANMessage(int mid, QByteArray msg) {
  ...
  case TLM_ID: {
    if (msg.at(0) == 0x01) {
      carStatus->setTelemetryStatus(1);
    } else if (msg.at(0) == 0x00) {
      carStatus->setTelemetryStatus(0);
    }
    break;
  }
...
// Destroy, BOOM!
Canbus::~Canbus() { qDebug() << "Closing CAN..."; }
