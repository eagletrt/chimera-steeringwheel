...
#define WHEEL_RADIUS 0.1975

// Set Speed and emit Property
void CarStatus::setSpeed(QByteArray msg) {
  counterSpeed++;
  speedPrescaler = 20;
  // 0x07 encoder sinistro in teoria
  if (counterSpeed >= pedalsPrescaler) {
    counterSpeed = 0;
    int speed = (msg.at(0) << 16) + (msg.at(1) << 8) + msg.at(2);
    if (msg.at(7) == 1) {
      speed *= -1;
    }

    m_speed = floor((speed * WHEEL_RADIUS * 3.6) / 10000.);
...
void CarStatus::setKm(QByteArray msg) {
  int km = (msg.at(0) << 16) + (msg.at(1) << 8) + msg.at(2);
  m_km = floor(km / 1000.);
...
void CarStatus::changePump(int pumpID) {
  ...
  if (pumpID != LOOP_THROUGH_PUMPS) {
    ...
    // Loop through presents from 1 to 8 --> Case Single Button for Preset
    // change
    m_pump += 1;
...
// Set the value for the Pump
void CarStatus::changeEcuControl(int ecuControlID) {
  if (ecuControlID != LOOP_THROUGH_ECU_CONTROLS) {
    m_ecu_control = ecuControlID;
  } else {
    // Loop through presents from 1 to 8 --> Case Single Button for Preset
    // change
    m_ecu_control += 1;
    m_ecu_control = m_ecu_control > ECU_CONTROL_NUMBER ? m_ecu_control % ECU_CONTROL_NUMBER : m_ecu_control;
  }
  // emit pumpChanged();
}
...
QString CarStatus::CANStatus() const {
  ...
  return QString("%1%2%3%4%5%6%7%8")
      .arg(QString::number(m_invr), QString::number(m_invl),
           QString::number(m_front), QString::number(m_central),
           QString::number(m_pedals), QString::number(m_rear),
           QString::number(m_lv), QString::number(m_hv));
}
...
QString CarStatus::ERRStatus() const {
  ...
  return QString("%1%2%3%4%5%6%7%8%9")
      .arg(QString::number(m_err_apps), QString::number(m_err_bse),
           QString::number(m_err_wheel_left),
           QString::number(m_err_wheel_right), QString::number(m_err_steer),
           QString::number(m_err_imu_front), QString::number(m_err_gps),
           QString::number(m_err_imu_central), QString::number(m_err_imu_rear));
}
...
int CarStatus::getCtrlIsEnabled() { return m_ctrlIsEnabled; }
...
int CarStatus::carStatus() { return m_car_status; }

int CarStatus::getCurrentStatus() { return m_car_status; }

int CarStatus::getPump() { return m_pump - 1; }

int CarStatus::getEcuControl() { return m_ecu_control - 1; }

int CarStatus::getMap() { return m_preset - 1; }
...
void CarStatus::setTelemetryStatus(bool status) {
...
int CarStatus::pump() const { return m_pump; }
int CarStatus::preset() const { return m_preset; }
int CarStatus::ecuControl() const { return m_ecu_control; }
int CarStatus::velocity() const { return m_velocity; }
int CarStatus::brakeVal() const { return m_brakeVal; }
int CarStatus::throttleVal() const { return m_throttleVal; }
int CarStatus::speed() const { return m_speed; }
int CarStatus::km() const { return m_km; }
int CarStatus::invSxTemp() const { return m_invSxTemp; }
int CarStatus::invDxTemp() const { return m_invDxTemp; }
int CarStatus::hvTemp() const { return m_hvTemp; }
int CarStatus::lvTemp() const { return m_lvTemp; }
int CarStatus::hvVolt() const { return m_hvVolt; }
int CarStatus::lvVolt() const { return m_lvVolt; }
int CarStatus::getCtrlIsOn() { return m_ctrlIsOn; }
int CarStatus::warning() const { return m_warning; }
int CarStatus::error() const { return m_error; }
bool CarStatus::telemetryStatus() const { return m_telemetryStatus; }
// Destroy, BOOM!
CarStatus::~CarStatus() { qDebug() << "Closing CarStatus..."; }
