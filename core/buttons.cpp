...
#define ECU_CONTROL_1 100
#define ECU_CONTROL_2 101
#define ECU_CONTROL_3 102
#define ECU_CONTROL_4 103
#define ECU_CONTROL_5 104
#define ECU_CONTROL_6 105
...
void Buttons::readGPIOState() {
  ...
  for (int i = 0; i < pinEnabled.size(); i++) {
    ...
    if (pinState.at(i) != previusPinState.at(i)) {
      ...
      if ((map != -1 && map != oldMap) || (pump != -1 && pump != oldPump) || (ecuControl != -1 && ecuControl != oldEcuControl)) {
        ...
        if (!switchIsWrong) {
          ...
          if (ecuControl != -1 && ecuControl != oldEcuControl) {
            ecuControlChanged(ecuControl);
            oldEcuControl = ecuControl;
          }
        }
